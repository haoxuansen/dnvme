...
static int dnvme_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    LOG_ERR("dnvme_probe...");
    // LOG_NRM("YM %d %d %d", num_possible_cpus(), num_present_cpus(), num_active_cpus());
    ...
    //     LOG_ERR("csts: 0x%x", csts);
    // }
    // else
    // {
    //     LOG_ERR("pcie is disable");
    // }
    // LOG_DBG("cmbsz ofst:%#lx",offsetof(struct nvme_ctrl_reg,cmbsz));
    // LOG_DBG("cmbloc ofst:%#lx",offsetof(struct nvme_ctrl_reg,cmbloc));
    // LOG_DBG("bpinfo ofst:%#lx",offsetof(struct nvme_ctrl_reg,bpinfo));
    // LOG_DBG("bprsel ofst:%#lx",offsetof(struct nvme_ctrl_reg,bprsel));
    // LOG_DBG("bpmbl ofst:%#lx",offsetof(struct nvme_ctrl_reg,bpmbl));
    // LOG_DBG("cmbmsc ofst:%#lx",offsetof(struct nvme_ctrl_reg,cmbmsc));
    // LOG_DBG("cmbsts ofst:%#lx",offsetof(struct nvme_ctrl_reg,cmbsts));

    // LOG_DBG("pmrcap ofst:%#lx",offsetof(struct nvme_ctrl_reg,pmrcap));
    // LOG_DBG("pmrctl ofst:%#lx",offsetof(struct nvme_ctrl_reg,pmrctl));
    // LOG_DBG("pmrsts ofst:%#lx",offsetof(struct nvme_ctrl_reg,pmrsts));
    // LOG_DBG("pmrebs ofst:%#lx",offsetof(struct nvme_ctrl_reg,pmrebs));
    // LOG_DBG("pmrswtp ofst:%#lx",offsetof(struct nvme_ctrl_reg,pmrswtp));
    // LOG_DBG("pmrmscl ofst:%#lx",offsetof(struct nvme_ctrl_reg,pmrmscl));
    // LOG_DBG("pmrmscu ofst:%#lx",offsetof(struct nvme_ctrl_reg,pmrmscu));
    // LOG_DBG("sqtdbl ofst:%#lx",offsetof(struct nvme_ctrl_reg,sqtdbl));
    // LOG_DBG("sqhdbl ofst:%#lx",offsetof(struct nvme_ctrl_reg,sqhdbl));
    ...
    unsigned long ioctl_param)
{
    ...
    case NVME_IOCTL_DEVICE_STATE:
        ...
        case ST_NVM_SUBSYSTEM:
            LOG_DBG("Performing NVM Subsystem reset");
