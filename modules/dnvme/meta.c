	...
	struct nvme_meta_compare __user *ucmp)
{
	...
	meta1 = dnvme_find_meta(ndev, cmp.id1);
	meta2 = dnvme_find_meta(ndev, cmp.id2);
	if (!meta1 || !meta2) {
	...
	if (meta1->size != meta2->size) {
		dnvme_err(ndev, "meta size is different!(%u vs %u)\n",
			meta1->size, meta2->size);
		return -EPERM;
	}

	if (!test_bit(NVME_META_F_BUF_CONTIG, &meta1->flags) ||
		!test_bit(NVME_META_F_BUF_CONTIG, &meta2->flags)) {
		dnvme_err(ndev, "Not support SGL compare!\n");
		return -EPERM;
	}

	if (memcmp(meta1->buf, meta2->buf, meta1->size)) {
