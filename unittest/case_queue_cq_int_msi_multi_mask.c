...
void int_msi_signal_mask(void)
{
    ...
    LOG_INFO("case_queue_cq_int_msi_signal_mask case start\n");
    for (q_index = 0; q_index < index_max; q_index++)
    {
        ...
        {
            error_status = FAILED;
            LOG_ERROR("int_msi_signal_mask %d mask bit fail\n", q_index);
        }
    }
    if (error_status != 0)
    {
        test_flag = FAILED;
        LOG_ERROR("case_queue_cq_int_msi_signal_mask case result is -1!!!!!\n");
    }
    else
    {
        test_flag = SUCCEED;
        LOG_INFO("case_queue_cq_int_msi_signal_mask case result is 1!!!!!\n");
    }
    LOG_INFO("case_queue_cq_int_msi_signal_mask case end!!!\n");
}
static void int_msi_multi_mask(void)
{
    ...
    uint32_t error_status = 0;
    uint32_t msi_mask_flag = 0;
    LOG_INFO("case_queue_cq_int_msi_multi_mask case start\n");
    for (q_index = 0; q_index < index_max; q_index++)
    {
        test_flag = 0;
    ...
    {
        test_flag = FAILED;
        LOG_ERROR("case_queue_cq_int_msi_multi_mask case result is -1!!!!!\n");
    }
    else
    {
        test_flag = SUCCEED;
        LOG_INFO("case_queue_cq_int_msi_multi_mask case result is 1!!!!!\n");
    }
    LOG_INFO("case_queue_cq_int_msi_multi_mask case end!!!\n");
}
