...
1. 创建一个 .c 文件，通常在 unittest 或其合适的子目录下。
#. 添加 case 的入口函数，入口函数的格式如下：
	...
	.. code-block:: c
		...
		*/
		static int case_xxx(struct nvme_tool *tool, struct case_data *priv)
		{
			/* do something */
			return errno;
		}
		NVME_CASE_SYMBOL(case_xxx, "description");

#. 继续填充和完善 case 需要执行的内容即可。

.. note::
	1. 每个 case 彼此是独立的，即前一个 case 的执行不能影响到后面 case 的运行。
	#. 不同 case 文件中的函数仅限在当前 case 下使用，不能互相调用。 

How to view result
------------------

| 测试结束后可通过以下两种方式来确认相应的 case 执行是否成功。

Method 1: Search terminal log
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

| 直接搜索 Terminal 终端打印的 log 信息。

| 每个 case 执行结束后，都会在终端打印其执行结果：PASS/FAIL/SKIP。

Method 2: Parse json file
^^^^^^^^^^^^^^^^^^^^^^^^^

| 解析测试报告内容。

| case 执行过程中，会有一个独立的模块记录测试的步骤、结果等信息。最终测试结束退出时，会生成一份 json 格式的测试报告。用户可以使用配套的工具解析测试报告的内容，最终会以列表的形式展现每个 case 的执行结果。

| 若想了解某个 case 执行的详细信息，还可以点击相应 case 查看。

.. tab-set::

	.. tab-item:: Case

		.. image:: ./image/py_tool_case.png
			:align: center
			:width: 75%
	
	.. tab-item:: Subcase

		.. image:: ./image/py_tool_subcase.png
			:align: center
			:width: 75%
	
	.. tab-item:: Details

		.. image:: ./image/py_tool_subcase_detail.png
			:align: center
