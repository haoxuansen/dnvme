...
static void int_msi_multi_mask(void)
{
    ...
    {
        test_flag = -1;
        pr_err("case result is -1!!!!!\n");
    }
    else
    {
        test_flag = 0;
        pr_info("case result is 1!!!!!\n");
    }
    pr_info("case end!!!\n");
}
...
static int case_queue_cq_int_msi_multi_mask(struct nvme_tool *tool, struct case_data *priv)
{
	struct nvme_dev_info *ndev = tool->ndev;
...
NVME_CASE_SYMBOL(case_queue_cq_int_msi_multi_mask, "?");
