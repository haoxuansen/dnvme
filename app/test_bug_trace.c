...
dword_t iocmd_cstc_rdy_test(void)
{
    ...
    LOG_COLOR(PURPLE_LOG, "tests iocmd read csts.rdy will hang at pcie gen1/gen2 \r\n");
...
{
    LOG_COLOR(RED_LOG, "tests device's dbl will error bug \r\n");

    dword_t u32_tmp_data = ioctl_read_data(file_desc, 0x1620, 4);
    LOG_COLOR(PURPLE_LOG, "read 0x1620: %x\n", u32_tmp_data);
    ...
    LOG_COLOR(PURPLE_LOG, "after write,read 0x1620: %x\n", u32_tmp_data);
    for (int i = 0; i < 0x3000; i += 4)
    {
        u32_tmp_data = ioctl_read_data(file_desc, i, 4);
        LOG_COLOR(PURPLE_LOG, "r ofst %x=%x\n", i, u32_tmp_data);
    }
    return SUCCEED;
}